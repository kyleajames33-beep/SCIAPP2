...
export function BattleArena({
  ...
  // Extract base path (e.g., "/sprites/player_strong" from "/sprites/player_strong_idle.png")
  ...
  // Particle colors based on boss phase
  const particleColors = isPhase3
    ? ['#ef4444', '#dc2626', '#f87171', '#fca5a5', '#b91c1c', '#991b1b', '#fee2e2', '#fecaca']
    : isPhase2
    ? ['#f97316', '#ea580c', '#fb923c', '#fdba74', '#c2410c', '#9a3412', '#fed7aa', '#ffedd5']
    : ['#6366f1', '#4f46e5', '#818cf8', '#a5b4fc', '#4338ca', '#3730a3', '#c7d2fe', '#e0e7ff'];

  // Pre-calculated particle directions (8 particles spread evenly)
  const particles = Array.from({ length: 8 }, (_, i) => {
    const angle = (i / 8) * Math.PI * 2;
    const distance = 60 + (i % 3) * 30; // Vary distance: 60, 90, 120
    return {
      x: Math.cos(angle) * distance,
      y: Math.sin(angle) * distance,
      size: 6 + (i % 3) * 4, // Vary size: 6, 10, 14
      color: particleColors[i],
    };
  });

  // Calculate "Distance Traveled" for parallax
  const distance = totalQuestionsAnswered * 50;

  // Speed multiplier based on streak
  const isSpeeding = playerStreak >= 5;
  const speedDuration = isSpeeding ? 0.25 : 0.5; // 2x faster when speeding

  return (
    <motion.div
      animate={{
        borderColor: isPhase3 ? '#ff0000' : isPhase2 ? '#f97316' : '#6366f1',
    ...
    >
      ...
      {/* PHASE 3 SCREEN SHAKE (Constant subtle vibration) */}
      {isPhase3 && (
        <motion.div
      ...
      {/* LAYER 3: The Moving Floor (Speedlines) - Speeds up during streak */}
      <div className="absolute bottom-0 w-full h-32 bg-gradient-to-t from-indigo-900/40 to-transparent overflow-hidden">
        ...
        {/* Additional faster speedlines when speeding */}
        {isSpeeding && (
          <motion.div
            animate={{ x: [-150, 0] }}
      ...
      <div className="flex justify-between items-end h-full px-20 pb-16 relative z-10">
        ...
        {/* PLAYER */}
        <motion.div
          animate={lastAction === 'attack' ? { x: [0, 200, 0], scale: [1, 1.2, 1] } : { y: [0, -5, 0] }}
        ...
        {/* BOSS */}
        {/* Outer wrapper: handles action-triggered animations (hurt/attack) */}
        <motion.div
          key={lastAction || 'idle'}
          animate={
            lastAction === 'attack'
              ? {
                  // HURT: Player hit the boss — shake and flash
                  x: [0, -10, 10, -10, 10, 0],
                  filter: [
                    'brightness(1)',
                    'brightness(2.5)',
              ...
              : lastAction === 'miss'
              ? {
                  // ATTACK: Boss lunges toward the player
                  x: [0, -50, 0],
                  scale: [1, 1.2, 1]
                }
              : {}
          ...
          className="relative"
        >
          {/* Inner wrapper: constant idle hover animation */}
          <motion.div
            animate={
              isPhase3
                ? {
          ...
          >
            ...
            >
              <img
                src={`/sprites/${subject.toLowerCase()}_boss.png`}
                alt="Boss"
                className="w-full h-full object-contain drop-shadow-2xl"
              />
      ...
      <div className="absolute top-4 left-4 bg-black/50 px-3 py-1 rounded-full border border-indigo-400/30">
        <p className="text-xs text-indigo-300 font-bold">
          Distance: {totalQuestionsAnswered * 50}m
        </p>
      </div>
    </motion.div>
  );
